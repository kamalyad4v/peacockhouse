...
export default function Peacock3D({ scrollProgressRef }: Peacock3DProps) {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const scrollT = scrollProgressRef ? ('current' in scrollProgressRef ? scrollProgressRef.current : 0) : 0;
...
function CameraRig({ scrollProgressRef }: Peacock3DProps) {
  ...
  useFrame(() => {
    const t = scrollProgressRef ? ('current' in scrollProgressRef ? scrollProgressRef.current : 0) : 0;
